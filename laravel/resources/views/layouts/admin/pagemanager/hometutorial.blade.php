...
<button id="btn-add" class="btn btn-success mt-3 mb-2">Add Tutorial</button>
<form>
        ...
        <table class="table table-striped">
            ...
            <tbody id="list_bahasa">          
                @foreach ($collectionLinkBahasa as $item)
                    <tr id="{{$item->id_link}}" class="{{$item->id_bahasa}} bahasa">
                        <td style="width: 5%">{{$item->id_link}}</td>
                        <td id="title_{{$item->id_link}}" style="width: 30%">{{$item->title}}</td>
                ...
                <form id="myForm" name="myForm" class="form-horizontal" novalidate="">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="title" name="title"
                            placeholder="Masukan Title" value="">
...
@section('custom_js')
    <script src="{{asset('js/crudajaxlinkhtutorial.js')}}"></script>
@endsection
